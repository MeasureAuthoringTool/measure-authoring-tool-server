...
public class SecurityQuestionVerifier {
	...
	public SecurityQuestionVerifier(String question1, String answer1,
		...
		if(!checkForMarkUp(answer1, answer2, answer3)){
			message.add(MessageDelegate.getInstance().getNoMarkupAllowedMessage());
		} 
		...
		if(questionTexts.size() < 3) {
			message.add(MessageDelegate.getInstance().getNotUniqueQuestions());
		}
		
		if(message.size() > 0) {
			message.add(0, MessageDelegate.getInstance().getSecurityQuestionHeeaderMessage());
		}
