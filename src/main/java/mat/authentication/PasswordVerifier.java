...
public class PasswordVerifier {
	...
	public PasswordVerifier(String userId, String password, String confirmPassword) {
		...
		message.add(MessageDelegate.getInstance().getDoesntFollowRulesMessage());
		if(!confirmed) {
			message.add(MessageDelegate.getInstance().getPasswordMismatchMessage());
		}
		if(!notTooShort || !notTooLong) {
			message.add(MessageDelegate.getInstance().getPasswordWrongLengthMessage());
		}
		if(!containsUpper) {
			message.add(MessageDelegate.getInstance().getMustContainUpperMessage());
		}
		if(!containsLower) {
			message.add(MessageDelegate.getInstance().getMustContainLowerMessage());
		}
		if(!containsNumber) {
			message.add(MessageDelegate.getInstance().getMustContainNumberMessage());
		}
		if(!containsSpecial) {
			message.add(MessageDelegate.getInstance().getMustContainSpecialMessage());
		}
		if(!notUserid) {
			message.add(MessageDelegate.getInstance().getMustNotContainLoginIdMessage());
		}
		if(!noRepeatedChar) {
			message.add(MessageDelegate.getInstance().getMustNotContainRunsMessage());
		}
	}
