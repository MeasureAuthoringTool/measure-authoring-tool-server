...
import mat.service.authentication.TwoFactorValidationService;
import mat.authentication.HashUtility;
import mat.util.StringUtility;
import org.springframework.stereotype.Service;

@Service
public class LoginCredentialServiceImpl implements LoginCredentialService {
	private static final Logger logger = LogManager.getLogger(LoginCredentialServiceImpl.class);
	...
	public boolean changePasswordSecurityAnswers(LoginModel model) {
		...
		if (userDetails != null) {
			...
			model.setErrorMessage(MessageDelegate.getInstance().getGenericErrorMessage());
			result = false;
		}
		return result;
	}
	...
	private LoginModel firstFailedLogin(String userId, int currentPasswordlockCounter, LoginModel validateUserLoginModel, MatUserDetails validateUserMatUserDetails) {
		...
		validateUserLoginModel.setErrorMessage(MessageDelegate.getInstance().getLoginFailedMessage());
	...
	private LoginModel isAlreadySignedIn(String userId, LoginModel validateUserLoginModel) {
		...
		validateUserLoginModel.setErrorMessage(MessageDelegate.getInstance().getLoginFailedAlreadyLoggedInMessage());
		validateUserLoginModel.setLoginFailedEvent(true);
		return validateUserLoginModel;
	}
	...
	private LoginModel isValidUserDetailsNotNull(String userId, String password, LoginModel validateUserLoginModel, MatUserDetails validateUserMatUserDetails) {
		...
		if (validateUserMatUserDetails.getStatus().getStatusId().equals("2")) {
			...
			validateUserLoginModel.setLoginFailedEvent(true);
			validateUserLoginModel.setErrorMessage(MessageDelegate.getInstance().getAccountRevokedMessage());
	...
	private LoginModel isPasswordMatched(String userId, LoginModel validateUserLoginModel, MatUserDetails validateUserMatUserDetails) {
		...
		if (!validateUserMatUserDetails.getUserPassword().isInitial() && !validateUserMatUserDetails.getUserPassword().isTemporaryPassword()) {
			token = setAuthenticationToken(validateUserMatUserDetails);

		}
		validateUserLoginModel = loginModelSetter(validateUserLoginModel, validateUserMatUserDetails);
		validateUserLoginModel.setUsernamePasswordAuthenticationToken(token);
		hibernateUserService.saveUserDetails(validateUserMatUserDetails);
	...
	private LoginModel isValidLogin(String userId, String password, LoginModel validateUserLoginModel, MatUserDetails validateUserMatUserDetails) {
		...
		boolean alreadySignedIn = MatContextServiceUtil.isAlreadySignedIn(lastSignOut, lastSignIn, currentTimeStamp);
		if (alreadySignedIn) {
	 ...
	 */
	@Override
	public LoginModel isValidUser(String userId, String password, String oneTimePassword,String sessionId) throws BadCredentialsException{
		...
		if (!validateUserLoginModel.isLoginFailedEvent()) {
			...
			throw new BadCredentialsException(validateUserLoginModel.getErrorMessage());
		}
		return validateUserLoginModel;
	}
	...
	private LoginModel isValid2FactorOTP(String userId, String oneTimePassword, LoginModel validateUserLoginModel, MatUserDetails validateUserMatUserDetails) {
		...
		if(this.matOtpValidatorService != null){
			...
			if(!isValidOTP){
				...
				loginModel.setErrorMessage(MessageDelegate.getInstance().getLoginFailedMessage());
			}
		}else{
			loginModel.setLoginFailedEvent(true);
			loginModel.setErrorMessage(MessageDelegate.getInstance().getLoginFailedMessage());
	...
	private UsernamePasswordAuthenticationToken setAuthenticationToken(MatUserDetails userDetails) {
		logger.debug("Setting authentication token");
		UsernamePasswordAuthenticationToken auth = new UsernamePasswordAuthenticationToken(userDetails.getId(), userDetails.getUserPassword().getPassword(), userDetails.getAuthorities());
	...
	private LoginModel temporaryPasswordExpiration(String userId, LoginModel validateUserLoginModel, MatUserDetails validateUserMatUserDetails) {
		...
		if (calendarForToday.after(calendar)) {
			...
			validateUserLoginModel.setErrorMessage(MessageDelegate.getInstance().getLoginFailedTempPasswordExpiredMessage());
