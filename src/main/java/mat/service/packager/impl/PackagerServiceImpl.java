...
import mat.validation.MeasurePackageClauseValidator;
...
public class PackagerServiceImpl implements PackagerService {
	...
	/** The Constant logger. */
	private static final Logger logger = LogManager.getLogger(PackagerServiceImpl.class);
	...
	public MeasurePackageOverview buildOverviewForMeasure(String measureId) {
		...
		List<String> allowedPopulationsInPackage = MeasureUtil.getInstance().getAllowedPopulationsInPackage();
		// Load Measure Xml
	 ...
	 * (non-Javadoc)
	 * 
	 * @see mat.service.packager.PackagerService#delete(mat.client.measurepackage.
	 * MeasurePackageDetail)
	 */
	@Override
	...
	public MeasurePackageSaveResult save(MeasurePackageDetail detail, String loggedInUserId, String scoring) {
		...
		List<String> messages = clauseValidator.isValidMeasurePackage(detail.getPackageClauses(), scoring);
		...
		if (messages.size() == 0) {
			...
			for (String message : messages) {
				logger.info("Server-Side Validation failed for MeasurePackageClauseValidator for Login ID: "
						+ loggedInUserId + " And failure Message is :" + message);
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see mat.service.packager.PackagerService#saveRiskAdjVariables(mat.client.
