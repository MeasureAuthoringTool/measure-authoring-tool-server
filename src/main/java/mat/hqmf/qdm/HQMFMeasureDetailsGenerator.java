...
import mat.service.impl.XMLUtility;
...
public class HQMFMeasureDetailsGenerator implements Generator {
	...
	/**
	 * This method will add a new tag
	 * '<measureReleaseVersion releaseVersion={version}/>' right under the
	 * '<measure>' tag. This will contain the QDM version used in the measure. If
	 * Measure Release Version is 'v4', then QDM Version is '4.1.2'; else it is
	 * '4.3' The 'new_measureDetails.xsl' will then read the
	 * '<measureReleaseVersion releaseVersion={version}/>' tag and add a comment
	 * with the value of 'releaseVersion' attribute in it.
	 * 
	 * @param simpleXML
	...
	private String addReleaseVersionToSimpleXML(String simpleXML, String releaseVersion) {
		...
		if (measureDetailsTagIndex > -1) {
			simpleXML = simpleXML.substring(0, measureDetailsTagIndex) + "<measureReleaseVersion releaseVersion=\""
					+ releaseVersion + "\"/>" + simpleXML.substring(measureDetailsTagIndex);
		}
	 ...
	 *
	 * @param measureDetailsHQMF_XML
	 *            the measure details hqm f_ xml
	 * @return the string
	 */
	 ...
	 *
	 * @param measureDetailsXml
	 *            the measure details xml
	 */
	private void editDefaultStartDate(XmlProcessor measureDetailsXml) {
		...
		try {
			...
			if (measurementPeriodLowNode != null) {
				String lowDateValue = measurementPeriodLowNode.getAttributes().getNamedItem("value").getNodeValue();
				measurementPeriodLowNode.getAttributes().getNamedItem("value").setNodeValue(getLowValue(lowDateValue));
			}
		} catch (XPathExpressionException e) {
			e.printStackTrace();
		}
